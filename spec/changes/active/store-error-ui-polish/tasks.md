# Tasks

## Implementation

- [x] 1. Refine About settings preview-channel enabled-state UI
- [x] 2. Update Skill Store rate-limit messaging and banner layout
- [x] 3. Reuse the corrected rate-limit guidance in CreateSkillModal
- [x] 4. Add or update tests
- [x] 5. Update implementation record
- [x] 6. Classify remote GitHub store failures into rate-limit / network / invalid-repo guidance

## Verification

- [x] Run targeted desktop tests
- [x] Run desktop lint
- [ ] Sync stable specs if behavior contract changed
